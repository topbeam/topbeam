...
test('darwin dışı platformda hiç denemez → false', async () => {
  const ok = await notifyMac('Ocean', 'selam', { platform: 'linux', bin: '/usr/bin/true' });
  assert.equal(ok, false);
});

test('binary yoksa hata YUTULUR → false (fırlatmaz)', async () => {
  const ok = await notifyMac('Ocean', 'selam', {
    platform: 'darwin',
    bin: '/boyle-bir-binary-yok-ocean-test',
  });
  assert.equal(ok, false);
});

test('komut başarılıysa true (sahte bin: /usr/bin/true)', async () => {
  const ok = await notifyMac('Ocean', 'ürün geliştirildi 🎉', { platform: 'darwin', bin: '/usr/bin/true' });
  assert.equal(ok, true);
});

test('komut hata verirse false (sahte bin: /usr/bin/false)', async () => {
  const ok = await notifyMac('Ocean', 'selam', { platform: 'darwin', bin: '/usr/bin/false' });
  assert.equal(ok, false);
});
...
test('OCEAN_NO_NOTIFY=1 → hiç çalışmaz, false', async () => {
  ...
  try {
    const ok = await notifyMac('Ocean', 'selam', { platform: 'darwin', bin: '/usr/bin/true' });
    assert.equal(ok, false);
  } finally {
...
test('tırnaklı mesaj AppleScript stringini bozmaz (true bin ile sorunsuz)', async () => {
  const ok = await notifyMac('Ocean "test"', 'mesaj "tırnaklı" \\ ters bölü', {
    platform: 'darwin',
    bin: '/usr/bin/true',
  });
  assert.equal(ok, true);
